...
setDeleteVideoResponse(true);
  ...
  return (
    <div>
       ...
       <Card style={{ width: '18rem' }} draggale onDragStart={e=>dragStarted(e,video.id)}>
      <Card.Img variant="top" src={video.url} onClick={handleShow} style={{width:'100%', height:'150px'}} />
      <Card.Body>
        <div className='d-flex justify-content-between align-items-center' >
        <h5>{video.title}</h5>
        <button className='btn' onClick={()=>removeVideo(video?.id)}><i class="fa-solid fa-chevron-up" style={{color: "#0e0c0c"}}></i></button>
        </div>
